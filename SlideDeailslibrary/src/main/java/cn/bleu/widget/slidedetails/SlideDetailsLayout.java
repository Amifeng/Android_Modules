...
public class SlideDetailsLayout extends ViewGroup {
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int i = 0; i < vGroup.getChildCount(); i++) {
                ...
                if (child instanceof ScrollView) {
                    result = canScrollViewSroll((ScrollView) child,direction);
    ...
    protected boolean canScrollViewSroll(ScrollView absListView,int direction) {
        if (mStatus == Status.OPEN) {
...
//                    .getTop() + "=== " + absListView.getPaddingTop());
            ...
            if (childView.getMeasuredHeight()<=absListView.getScrollY()+absListView.getHeight()){
                View child = absListView.getChildAt(0);
                if (child instanceof View) {
                    return ViewCompat.canScrollVertically(child, direction);
                } else {
                    return innerCanChildScrollVertically(child, direction);
                }
            }else{
                final int count = absListView.getChildCount();
                return count > 0
